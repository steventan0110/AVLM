...
from src.models.avlm_emo import AVLMForEmoModel
from src.data_utils.iemocap_data_util import AVLMFinetuneDataModule
...
class ModelArguments:
    ...
    test_gen: Optional[bool] = field(default=False)
    force_emo_pred: Optional[int] = field(default=None)
...
def init_model(args, ckpt_path=None, tokenizer=None):
    if ckpt_path is not None:  
        # model = AVLMForEmoModel(hparams=vars(args), tokenizer=tokenizer)
        model = AVLMForEmoModel.load_from_checkpoint(
...
class VisualSpiritLMTrainer:
    def __init__(self, args: ModelArguments) -> None:
        ...
        self.data_module = AVLMFinetuneDataModule(
            tokenizer=tokenizer,
